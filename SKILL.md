...
DRE (Doll Reconstruction Engineer) converts reference character images into standardized 1.8-head vinyl toy doll figures. The workflow follows a strict **Shape > Pose > Appearance** priority with iterative generation (≤5 rounds) and final scoring.

Core principles:
- **Shape** (template authority): head-to-body ratio, limb proportions, facial blankness, material — never modified
...
The output is a vinyl toy figure that preserves the reference character's identity while conforming to the standard doll template's structural constraints.
...
Always use `vision_analyze` for single-image analysis. Never use `browser_vision` for inspection (kimi-k2.6 hallucinates on side-by-side images). `browser_vision` is only for presenting the three-image comparison UI.

Core pattern: `vision_analyze` each image individually → agent compares text descriptions → write report to `inspection.html`.
...
**双图策略要点**（详见 `references/strategy-dual-image.md`）：
- 模板图 + 参考图同时传入
- 提示词用 Image 编号标注角色分工
...
4. 复制模板图和参考图到 r{N}/
5. 保存本轮 prompt 到 r{N}/prompt.md
6. 写等待态 HTML（三图并列 + ASCII 进度条），browser_navigate 打开
...
步骤 6 写入**等待态模板**（三图并列 + ASCII 进度条），步骤 8 完成后替换为实际报告。

Use the template at `templates/inspection.html`:
1. Copy the template to `r{N}/inspection.html`
2. Replace `{N}` placeholders with the actual round number
3. Replace image paths with actual `file://` paths to `r{N}/` copies
4. After inspection completes, replace the `<div class="waiting">...</div>` block with the actual report HTML

> 步骤 6：写入等待态 HTML → `browser_navigate` → 用户看到三图 + 进度条
> 步骤 7：三次 `vision_analyze` 逐图分析
> 步骤 8：agent 文字对比，生成报告
> 步骤 9：报告替换 `<div class="waiting">...</div>` → `browser_navigate` → 完整检验页
...
1. 对每轮成品图调用 `vision_analyze` 单图分析，获取文字描述。
2. 对模板图和参考图各调用一次 `vision_analyze` 获取基准描述。
3. Agent 基于文字描述逐轮对比评分（Shape vs 模板描述，Pose/Appearance vs 参考描述）。
4. 禁止依赖之前轮次的检验报告文本 — 每轮评分必须基于本轮 vision_analyze 返回的实际描述。
...
| **Profile 缺少 auxiliary.vision 配置**：新创建的 profile 默认没有 vision provider，`vision_analyze` 报错 "No LLM provider configured for task=vision" | 审图 | 在 profile 的 `config.yaml` 中手动添加 `auxiliary.vision` 段（model/base_url/api_key），详见 `references/dual-reviewer-pattern.md` |
| **Kanban worker protocol violation**：worker 正常完成审图但直接退出，未调用 `kanban_complete()`，任务状态显示 crashed。Cris 的协调任务尤其容易触发——它创建了子任务但忘记完成自身，导致子任务卡在 `todo` | Kanban 派发 | 在**所有**任务 body 中显式写 "⚠️ 完成后必须调用 kanban_complete(summary='...', metadata={...})！不要直接退出！"。连锁故障时手动 `unblock` + `complete` 父任务 |
| **Kanban 子任务依赖链阻塞**：父任务 blocked 时，子任务永远 `todo`，不会自动激活 | Kanban 派发 | 确保父任务正常 `kanban_complete`（非 blocked/crashed），子任务才会被 `promoted` 到 `ready` |
| **delegate_task 可以加载 skill**：子代理通过 `skill_view()` 成功加载并遵循 skill 流程（已验证 systematic-debugging 四阶段流程），可用于审图等非生成类子任务 | 审图 | 子代理 toolsets 需包含 `skills`，在 context 中写 "先调用 skill_view('xxx') 加载 skill" |
| **Kanban worker protocol violation**：worker 正常完成审图但直接退出，未调用 `kanban_complete()`。根因是 chat 范式与 task queue 范式的冲突——模型默认"输出文本→结束"，kanban 要求"输出文本→kanban_complete→结束"。SOUL.md 和 task body 写"必须调用"无效——模型会在文本中确认"好的"，但文本本身就是 agent loop 终止信号 | Kanban 派发 | **SOUL.md 重定义"完成"**：写"你的工作流程不是'输出文本→结束'，而是'kanban_show→干活→kanban_complete'。文本只是中间产物，不是终点。" 已验证此写法使 cris/alice/bob 连续 4 个任务零 protocol violation。备选：`--goal` 模式、提高 `kanban.failure_limit`。详见 `references/kanban-orchestration.md` |
| **Kanban 子任务依赖链阻塞**：父任务 blocked/crashed 时，子任务永远 `todo`，不会自动激活 | Kanban 派发 | 确保父任务正常 `kanban_complete`（非 blocked/crashed），子任务才会被 `promoted` 到 `ready`。已发生时手动 `unblock` + `complete` 父任务 |
| **Profile 缺少 plugins/ 目录**：新 profile 的 `HERMES_HOME` 下没有 `plugins/`，导致 vision_analyze、image_gen 等插件工具不可用 | Profile 配置 | `ln -s ~/.hermes/plugins ~/.hermes/profiles/<name>/plugins`，重启 Hermes 生效（插件为进程级缓存） |
...
| 回退方案 | `scripts/doubao_describe.py` | 当 auxiliary.vision 不可用时，直接调用 doubao-seed-2.0-pro |
| 高级方案 | Alice+Bob 双审图员 | 两个不同模型 agent 分别审图后交叉验证，避免单一模型幻觉（详见 `references/dual-reviewer-pattern.md`） |
| 持久化方案 | Kanban + Profiles | 跨 session 审图派发，crash 可恢复（详见 `references/kanban-orchestration.md`） |
...
`delegate_task` 子代理和 **Kanban worker** 均**无法调用 `image_gen` 插件**。原因：插件目录 (`plugins/`) 不跨 profile 共享——每个 profile 的 `HERMES_HOME` 不同，Hermes 在各自 `HERMES_HOME/plugins/` 下找插件。新创建的 profile 默认没有 `plugins/` 目录，需创建符号链接 `ln -s ~/.hermes/plugins ~/.hermes/profiles/<name>/plugins`，且插件为进程级缓存，需重启 Hermes 生效。

即使创建了符号链接，`image_gen` 工具在 Kanban worker 中仍不可用（已验证 cris profile 配置了 toolsets=image_gen + plugins 链接后，worker 报告工具列表中无 image_gen）。DRE 工作流中所有 `image_gen` 生成操作必须在**主会话**中直接执行。

但 `delegate_task` 子代理**可以**通过 `skill_view()` 加载 skill 并执行分析任务（已验证 systematic-debugging 四阶段流程），可用于审图等非生成类子任务。
...
| Step 2 | ≤7× `vision_analyze` | 5 轮成品 + 模板 + 参考，各 1 次 |
| **总计上限** | **≤5× `image_gen` + ≤22× `vision_analyze`** | |
...
- `references/doubao-vision-api.md` — doubao-seed-2.0-pro 视觉描述 API 配置与端点发现（替代 vision_analyze 的回退方案）
- `references/dual-reviewer-pattern.md` — **双审图员模式**：Alice (doubao-seed-2.0-pro) 审 Shape+Pose + Bob (kimi-k2-250905) 审 Appearance+交叉验证，多模型互补避免单一幻觉
- `references/kanban-orchestration.md` — **Kanban 多代理编排**：用 Hermes Kanban + Profiles 实现 DRE 审图的持久化并行派发，跨 session 可恢复
- `scripts/doubao_describe.py` — 直接调用 doubao-seed-2.0-pro 描述图片的独立脚本
